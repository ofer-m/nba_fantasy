...
plot_stats_ind <- function(gl, selected_player, category, date) {
  ...
  if (category == "3P") {
    ...
    plot <- gl_plot %>%
      ...
      geom_bar(fill = "#FE5A1D", width = 0.75) +
...
#' Function that returns table of summary statistics for two players
#' @param gl A tibble of gamelogs for NBA players
#' @param player_1 A string representing the first player to compare
#' @param player_2 A string representing the second player to compare
#' @param date A chr of string representing the start and end date
#' @return tibble

#' @examples
#' gamelogs <- read_csv("../data/nba_gamelogs.csv")
#' compare_players_table(gamelogs, "LeBron James", "Kevin Durant", c("2020-01-22", "2021-03-01"))
compare_players_table <- function(gl, player_1, player_2, date) {
  selected_players <- c(player_1, player_2)
  ...
  agg_player <- agg %>% filter(Player %in% selected_players)
  agg_player <- as_tibble(cbind(nms = names(agg_player), t(agg_player)))
  agg_player %>% filter(nms == "Player")
  colnames(agg_player) <- agg_player[1, ]

  agg_player %>%
    rename(Stat = Player) %>%
    filter(!Stat %in% c("Player", "Season")) %>%
    mutate(Stat = case_when(
      Stat == "General_Position" ~ "General Position",
      Stat == "Games_Played" ~ "Games Played",
      Stat == "Games_Started" ~ "Games Started",
      Stat == "Games_Missed" ~ "Games Missed",
      TRUE ~ Stat
    )) %>%
    select(Stat, selected_players[1], selected_players[2])
}

#' Create plot of comparing how 2 NBA players performs in a stat category
#' compared to the rest of the league.
#' The following stat categories create bar plots:
#'    3P, PTS, REB, AST, STL, BLK, TOV
#' The following stat categories create scatter plots
...
#' @param gl A tibble of gamelogs for NBA players
#' @param selected_players A chr of strings representing the names of 2 NBA players
#' @param category A string representing a stat category (options: 3P, PTS, REB, AST, STL, BLK, TOV, FG%, FT%)
#' @param positions A vector with positions to compare to (options: F, C, G)
#' @param min_games A number representing the minimum games the players played to appear in plot
#' @param min_minutes A vector containing the minimum and maximum minutes per game a player played to appear in plot
...
#' @examples
#' gamelogs <- read_csv("../data/nba_gamelogs.csv")
#' plot_stat_league(gamelogs, c("LeBron James", "Kevin Durant"), "3P", c("F", "C", "G"), 20, c(20, 40), c("2020-01-22", "2021-03-01"))
plot_stat_league_compare <- function(gl, selected_players, category, positions, min_games = 0, min_minutes = c(0, 48), date) {
  agg <- gl %>%
    filter(Date >= date[1] & Date <= date[2] & General_Position %in% positions) %>%
    group_by(Player) %>%
    summarise(
  ...
  line_colors <- list()
  line_colors[[selected_players[1]]] <- "black"
  line_colors[[selected_players[2]]] <- "grey"
  line_colors[["League Average"]] <- "blue"
  ...
  if (category == "3P") {
    ...
    plot <- ggplot(agg_plot, aes(`3P`,
      ...
      geom_histogram(binwidth = 0.2, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `3P`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
      ggtitle(paste("3P Compared to League (per game)")) +
      labs(y = "Number of Players") +
      coord_cartesian(xlim = c(min_amt, max_amt)) +
    ...
    plot <- ggplot(agg_plot, aes(`PTS`,
      ...
      geom_histogram(binwidth = 2, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `PTS`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
      ggtitle(paste("PTS Compared to League in", szn, "NBA Season (per game)")) +
      labs(y = "Number of Players") +
      coord_cartesian(xlim = c(min_amt, max_amt)) +
    ...
    plot <- ggplot(agg_plot, aes(`REB`,
      ...
      geom_histogram(binwidth = 0.5, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `REB`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
      ggtitle(paste("REB Compared to League in (per game)")) +
      labs(y = "Number of Players") +
      coord_cartesian(xlim = c(min_amt, max_amt)) +
    ...
    plot <- ggplot(agg_plot, aes(`AST`,
      ...
      geom_histogram(binwidth = 0.5, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `AST`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
      ggtitle(paste("AST Compared to League in (per game)")) +
      labs(y = "Number of Players") +
      coord_cartesian(xlim = c(min_amt, max_amt)) +
    ...
    plot <- ggplot(agg_plot, aes(`STL`,
      ...
      geom_histogram(binwidth = 0.1, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `STL`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
      ggtitle(paste("STL Compared to League in (per game)")) +
      labs(y = "Number of Players") +
      coord_cartesian(xlim = c(min_amt, max_amt)) +
    ...
    plot <- ggplot(agg_plot, aes(`BLK`,
      ...
      geom_histogram(binwidth = 0.2, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `BLK`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
      ggtitle(paste("BLK Compared to League (per game)")) +
      labs(y = "Number of Players") +
      coord_cartesian(xlim = c(min_amt, max_amt)) +
    ...
    plot <- ggplot(agg_plot, aes(`TOV`,
      ...
      geom_histogram(binwidth = 0.2, boundary = 0, fill = "#FE5A1D", colour = "white") +
      geom_vline(data = agg_player, aes(xintercept = `TOV`, colour = Player)) +
      geom_vline(aes(xintercept = mean_amt, color = "League Average"), lty = 2) +
    ...
    agg_plot <- agg_plot %>% filter(!(Player %in% selected_players))

    plot <- ggplot(agg_plot, aes(x = `FT%`, y = FT, text = paste(
      "FT%:", round(`FT%`, 3), "</br>",
      "</br>FT:", round(`FT`, 2),
      "</br>Player:", Player
    ))) +
      geom_point(colour = "#FE5A1D") +
      geom_point(data = agg_player, aes(x = `FT%`, y = FT, colour = Player), shape = "star", size = 3) +
      geom_vline(aes(xintercept = mean_ftp, color = "League Average"), lty = 2) +
      scale_colour_manual("", values = line_colors) +
      scale_y_continuous(
        limits = c(min_ft, max_ft),
        breaks = seq(min_ft, max_ft, 1)
    ...
    plot <- ggplot(agg_plot, aes(
      ...
      geom_point(colour = "#FE5A1D") +
      geom_point(data = agg_player, aes(x = `FG%`, y = FG, colour = Player), shape = "star", size = 3) +
      geom_vline(aes(xintercept = mean_fgp, color = "League Average"), lty = 2) +
      scale_colour_manual("", values = line_colors) +
      scale_y_continuous(
        limits = c(min_fg, max_fg),
        breaks = seq(min_fg, max_fg, 1)
...
#' Create plot of distribution of particular stat category for 2 NBA players in a season
#' The following stat categories create bar plots:
#'    3P, PTS, REB, AST, STL, BLK, TOV
#' The following stat categories create scatter plots
#'    FG%, FT%
...
#' @examples
#' gamelogs <- read_csv("../data/nba_gamelogs.csv")
#' plot_stats_ind(gamelogs, c("LeBron James", "Kevin Durant"), "3P", c("2020-01-22", "2021-03-01"))
plot_stats_compare <- function(gl, selected_players, category, date) {
  # filter gamelog to find players and games they played
  gl_plot <- gl %>% filter(Player %in% selected_players & DNP == 0 &
    Date >= date[1] & Date <= date[2])
  szn <- unique(gl_plot$Season)
  ...
  if (category == "3P") {
    ...
    plot <- gl_plot %>%
      ggplot(aes(x = `3P`, y = ..count.., fill = Player, text = ..count..)) +
    ...
    plot <- gl_plot %>%
      ggplot(aes(x = `REB`, y = ..count.., fill = Player, text = ..count..)) +
      geom_bar(position = "dodge", width = 0.5) +
      theme_bw(base_size = 12) +
      coord_cartesian(xlim = c(min_reb, max_reb)) +
      scale_x_continuous(breaks = seq(min_reb, max_reb, 1)) +
    ...
    plot <- gl_plot %>%
      ggplot(aes(x = `AST`, y = ..count.., fill = Player, text = ..count..)) +
      geom_bar(position = "dodge", width = 0.5) +
      theme_bw(base_size = 12) +
      coord_cartesian(xlim = c(min_ast, max_ast)) +
      scale_x_continuous(breaks = seq(min_ast, max_ast, 1)) +
    ...
    plot <- gl_plot %>%
      ggplot(aes(x = `STL`, y = ..count.., fill = Player, text = ..count..)) +
      geom_bar(position = "dodge", width = 0.5) +
      theme_bw(base_size = 12) +
      coord_cartesian(xlim = c(min_stl, max_stl)) +
      scale_x_continuous(breaks = seq(min_stl, max_stl, 1)) +
    ...
    plot <- gl_plot %>%
      ggplot(aes(x = `BLK`, y = ..count.., fill = Player, text = ..count..)) +
      geom_bar(position = "dodge", width = 0.5) +
      theme_bw(base_size = 12) +
      coord_cartesian(xlim = c(min_blk, max_blk)) +
      scale_x_continuous(breaks = seq(min_blk, max_blk, 1)) +
    ...
    plot <- gl_plot %>%
      ggplot(aes(x = `TOV`, y = ..count.., fill = Player, text = ..count..)) +
      geom_bar(position = "dodge", width = 0.5) +
      theme_bw(base_size = 12) +
      coord_cartesian(xlim = c(min_tov, max_tov)) +
      scale_x_continuous(breaks = seq(min_tov, max_tov, 1)) +
    ...
    plot <- gl_plot %>%
      # group_by(Player, `cut_pts`) %>%
      # summarize(Count = n()) %>%
      ggplot(aes(x = `cut_pts`, y = ..count.., fill = Player, text = ..count..)) +
      geom_bar(position = "dodge", width = 0.5) +
      theme_bw(base_size = 12) +
      scale_fill_manual("", values = line_colors) +
      ggtitle(paste("PTS Distribution")) +
...
#' Create plot of comparing how 2 NBA players perform in a stat category
#' in each fantasy week
#' The following stat categories create bar plots:
#'    3P, PTS, REB, AST, STL, BLK, TOV
#' The following stat categories create line plots
...
#' @examples
#' week_plot(week_stat, "LeBron James", "Kevin Durant", "3P")
week_plot_compare <- function(week_stat, player_1, player_2, category) {
  gl_plot <- week_stat %>%
    filter(Player == player_1 | Player == player_2)

  line_colors <- list()
  line_colors[[player_1]] <- "black"
  line_colors[[player_2]] <- "grey"


  if (category == "PTS") {
    plot <- ggplot(gl_plot, aes(
      x = Week, y = PTS, fill = Player,
      text = paste(
        "PTS per Game:", round(`PTS`, 2), "</br>",
        "</br>Games Played:", Games
      )
    ...
    plot <- ggplot(gl_plot, aes(
      x = Week, y = `FG%`, colour = Player, group = 1,
      text = paste(
        "FG per Game:", round(`FG`, 2), "</br>",
        "</br>FG% in Week:", round(`FG%`, 3),
        "</br>Games Played:", Games
      ...
      geom_line() +
      geom_point(aes(size = `FG`, colour = Player)) +
      scale_colour_manual("", values = line_colors) +
      coord_cartesian(ylim = c(0, 1)) +
      theme_bw(base_size = 12) +
      labs(title = "FG% Per Week in 2020-2021 NBA Season")
    ...
    plot <- ggplot(gl_plot, aes(
      x = Week, y = `FT%`, colour = Player, group = 1,
      text = paste(
        "FT per Game:", round(`FT`, 2), "</br>",
        "</br>FT% in Week:", round(`FT%`, 3),
        "</br>Games Played:", Games
      ...
      geom_line() +
      geom_point(aes(size = `FT`, colour = Player)) +
      scale_colour_manual("", values = line_colors) +
      coord_cartesian(ylim = c(0, 1)) +
      theme_bw(base_size = 12) +
      labs(title = "FT% Per Week in 2020-2021 NBA Season")
